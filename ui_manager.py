...
# 打印更新后的列表 (可选，用于验证)
# print(preset_vocab_levels)

        self.vocab_level_combo.addItems(preset_vocab_levels)
        current_vocab = current_config.get("vocab_level", "6000词 (CET-6)")
        ...
        #preset_learning_goals = ["提升日常浏览英文网页与资料的流畅度", "学术论文阅读", "商务英语沟通", "自定义"]
        # 预设学习目标选项列表
...
# print(preset_learning_goals)
        ...
        current_diff = current_config.get("difficulty_level", "中高级 (B2): 复杂句，抽象主题，多样化句式结构") # 修正默认值以匹配列表
        ...
        #preset_lengths = ["15个单词左右 (短句)", "30个单词左右 (中长句)", "50个单词左右 (复杂句)", "自定义"]
        # 预设句子长度选项列表
        preset_lengths = [
            # 按单词数量细分，并关联典型语境
