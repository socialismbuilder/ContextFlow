...
class ConfigDialog(QDialog):
    ...
    def add_api_setting(self, basic_layout, current_config):
        ...
        self.model_name.setCompleter(self.model_completer)

        self.refresh_models_btn = QPushButton("刷新")
        self.refresh_models_btn.clicked.connect(lambda: self._refresh_model_list(force=True))
    ...
    def add_Preferences_setting(self, basic_layout, current_config):
        ...
        current_diff = current_config.get("difficulty_level",
                                          "中级 (B1): 并列/简单复合句，稍复杂话题，扩大词汇范围")  # 修正默认值以匹配列表
    ...
    def _refresh_model_list(self, force=False):
        """从 API 获取可用模型列表并填充下拉框"""

        # 如果已有缓存且不强制刷新，直接使用缓存
        if self.cached_models and not force:
            ...
            if not api_url or not api_key:
                QMessageBox.warning(self, "错误", "请先填写 API URL 和 API Key")
                return
        ...
        if current:
            index = self.model_name.findText(current)
            if index >= 0:
                self.model_name.setCurrentIndex(index)
            else:
                self.model_name.setEditText(current)

        # 更新补全器
        self.model_completer = QCompleter(models)
        self.model_completer.setFilterMode(Qt.MatchFlag.MatchContains)
        self.model_completer.setCaseSensitivity(Qt.CaseSensitivity.CaseInsensitive)
        self.model_name.setCompleter(self.model_completer)
    ...
    def on_vocab_level_changed(self):
        self._handle_custom_change(self.vocab_level_combo, 'vocab_level_custom',
                                   self._get_form_layout(self.vocab_level_combo))

    def on_learning_goal_changed(self):
        self._handle_custom_change(self.learning_goal_combo, 'learning_goal_custom',
                                   self._get_form_layout(self.learning_goal_combo))

    def on_difficulty_changed(self):
        self._handle_custom_change(self.difficulty_combo, 'difficulty_custom',
                                   self._get_form_layout(self.difficulty_combo))
    ...
    def save_and_close(self):
        ...
        new_config = {
            ...
            "learning_goal": self._get_combo_value(self.learning_goal_combo,
                                                   getattr(self, 'learning_goal_custom', None)),
        ...
        for key in ["custom_prompts", "preset_api_urls", "preset_vocab_levels", "preset_learning_goals",
                    "preset_difficulties", "preset_lengths"]:
